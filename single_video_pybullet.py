...
def adding_mesh_object(
    ...
    if texture_to_load is None:
        ...
        toy.get_material().set_roughness(0)
        toy.get_material().set_transmission(0)
        toy.get_material().set_specular(0)
        toy.get_material().set_base_color((1.0,1.0,1.0))  
        from os.path import exists
        if exists(texture_roughness_to_load):
             tex_rough = visii.texture.create_from_file(name+"_rough",texture_roughness_to_load)
             toy.get_material().set_roughness_texture(tex_rough)
...
for i_obj in range(int(opt.nb_distractors)):
    ...
    obj_to_load = toy_to_load + "/meshes/model.obj"
    texture_to_load = toy_to_load + "/materials/textures/texture.png"
    texture_roughness_to_load = toy_to_load + "/materials/textures/texture_roughness.png"
    ...
    adding_mesh_object(name, obj_to_load, texture_to_load,texture_roughness_to_load,model_info_path_distractor,debug=opt.debug)
    name_obj_defined = os.path.basename(os.path.dirname(opt.path_single_obj))
    #print(name_obj_defined)
...
if opt.path_single_obj is not None:
    for i_object in range(opt.nb_objects):
        ...
        # adding_mesh_object(f"{name_obj_defined}_{i_object}",
        #                    opt.path_single_obj,
        #                    None,
        #                    model_info_path,
        #                    scale=opt.scale,
    ...
    for i_obj in range(int(opt.nb_objects)):
        ...
        adding_mesh_object(name, obj_to_load, texture_to_load,None, model_info_path, scale=opt.scale, debug=opt.debug)
